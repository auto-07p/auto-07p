...
Axis::Axis(bool s3D, int ty, double mxValue, double miValue, int numTickers, char* aName, int wh, SbColor clr)
{
    show3D = s3D;
...
Axis::Axis(int ty, double mxValue, double miValue, int numTickers, char* aName, int wh, SbColor clr)
{
    set(ty, mxValue, miValue, numTickers, aName, wh, clr);
}


void Axis::set(int ty, double mxValue, double mnValue, int numTickers, char* aName, int wh, SbColor clr)
{
    type = ty;
    maxValue = mxValue;
    minValue = mnValue;
...
SoSeparator * Axis::drawAxisName(float pos)
{
...
SoSeparator * Axis::drawTicker(float pos, float height)
{
// define ticker
...
SoSeparator * Axis::drawScale(float pos, float scale)
{
...
//define transform for the scale
    SoTransform *tkXform = new SoTransform;
    if(which == -1)
    {
