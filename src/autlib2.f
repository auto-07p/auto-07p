...
C
C     ---------- ------
      SUBROUTINE SOLVBV(IFST,IAP,DET,PAR,ICP,FUNI,BCNI,ICNI,RDS,
...
C
     ...
     +        RLCUR,RLOLD,RLDOT,THL,IFST)
         IF(KWT.GT.1)THEN
            CALL MPISBV(IAP,PAR,ICP,NDIM,UPS,UOLDPS,UDOTPS,
...
C     ---------- ---------
     ...
     + ICNI,IAP,PAR,NPAR,ICP,AA,BB,CC,DD,FA,FC,
...
C
C     ---------- ---------
      SUBROUTINE SBVFUN(NDIM,NCOL,NCB,NCA,FUNI,IAP,PAR,ICP,
...
C
      ...
      ENDDO
      CALL FUNI(IAP,NDIM,U,UOLD,ICP,PAR,IFST*2,F,DFDU,DFDP)
      DO IB=0,NCOL
         WPLOC(IB)=WP(IB)/DTM
      ENDDO
      IF(IFST.EQ.1)THEN
