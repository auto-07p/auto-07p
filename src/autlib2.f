...
C     ** Time evolution computations (parabolic systems)
     ...
     +        RLCUR,RLOLD,RLDOT,THL,IFST)
         IF(KWT.GT.1)THEN
            CALL MPISBV(IAP,RAP,PAR,ICP,NDIM,UPS,UOLDPS,UDOTPS,
...
C$    NT = OMP_GET_NUM_THREADS()
     ...
     +   FUNI,ICNI,IAP,RAP,PAR,NPAR,ICP,A,B,C,D,DD,FA,
...
C     
       IF(NBC.GT.0)THEN
         DO I=1,NDIM
            ...
            UBC1(I)=UPS(I,NTNC)
         ENDDO
         CALL BCNI(IAP,NDIM,PAR,ICP,NBC,UBC0,UBC1,FBC,IFST*2,DBC)
         DO I=1,NBC
            FC(I)=-FBC(I)
            IF(IFST.EQ.1)THEN
               DO K=1,NDIM
...
C     ---------- ------
     ...
     + ICNI,IAP,RAP,PAR,NPAR,ICP,AA,BB,CC,DD,DDD,FA,FC,FCFC,
...
C
      ...
      DOUBLE PRECISION DD(NCB,*),RAP(*),UPS(NDIM,0:*),UOLDPS(NDIM,0:*)
...
C
     ...
     +        DDD(1,1,IAM),FA(1,I),FCFC(1,IAM),UPS(1,II),UOLDPS(1,II),
...
C     ---------- ---------
     ...
     + ICNI,IAP,RAP,PAR,NPAR,ICP,AA,BB,CC,DD,FA,FC,
...
C
          DO IC=1,NCOL
             ...
             ENDDO
             JJ=(J-1)*NCOL
             CALL SBVFUN(NDIM,NCOL,NCB,NCA,FUNI,IAP,RAP,PRM,ICP,
     +            AA(1,IC1,J),BB(1,IC1,J),FA(IC1,J),UPS(1,JJ),
     +            UOLDPS(1,JJ),DTM(J),WP(1,IC),WT(1,IC),DFDU,DFDP,
     +            U,UOLD,F,IFST,NLLV)
             IF(IFST.EQ.1)THEN
                DO K=0,NDIM-1
                   IAMAX(IC1+K)=
...
C
C     ---------- ---------
      SUBROUTINE SBVFUN(NDIM,NCOL,NCB,NCA,FUNI,IAP,RAP,PAR,ICP,
...
C
      ...
      DOUBLE PRECISION, INTENT(IN) :: DTM,PAR(*),WT(0:NCOL),WP(0:*)
...
C
C     ---------- ------
      SUBROUTINE SBVICN(NDIM,NINT,NCB,NCA,ICNI,IAP,PAR,ICP,CC,DD,FC,
...
C
      IF(NINT.GT.0)THEN
         DO I=1,NDIM
            ...
            UIP(I)=UPOLDP(I)
         ENDDO
         CALL ICNI(IAP,NDIM,PAR,ICP,NINT,UIC,UIO,UID,UIP,
     +        FICD,IFST*2,DICD)
         DO M=1,NINT
            IF(IFST.EQ.1)THEN
               DO I=1,NDIM
