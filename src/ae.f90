...
! Local
    ...
    ALLOCATE(STUD(NBIFX,NDIM),STU(NBIFX,NDIM),STLA(NBIFX),STLD(NBIFX))
...
! Initialize computation of the next bifurcating branch.
 ...
 2  CALL SWPNT(IAP,RAP,PAR,ICP,RDS,NBIFX,STUD,STU,STLA,STLD,RLCUR,RLDOT,U,UDOT)
    ...
    IF(ABS(ISP).GT.0)THEN
       ...
       IF(ITP.EQ.-1)THEN
          IF(ISP.GT.0)THEN
             ...
             NBIF=NBIF+1
             IAP(35)=NBIF
             CALL STBIF(IAP,AA,NBIFX,STUD,STU,STLA,STLD,RLCUR,U,THL,THU)
             RLP=0.d0
             RBP=0.d0
             REV=0.d0
          ELSE
...
!
! ---------- -----
  SUBROUTINE STBIF(IAP,AA,M1SB,STUD,STU,STLA,STLD,RLCUR,U,THL,THU)
...
! known branch at this point.
    ...
    DIMENSION STUD(M1SB,*),STU(M1SB,*),STLA(*),STLD(*),RLCUR(*),THL(*),THU(*)
