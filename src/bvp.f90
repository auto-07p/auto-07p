...
MODULE BVP
  ...
  INTEGER, PARAMETER :: STPLBV_WRTBV8=2
...
! Store U-prime (derivative with respect to time or space variable).
    ...
    IF(MPISTATE==STPLBV_WRTBV8)THEN
       CALL MPIGAT(UDOTPS,NDIM*NCOL,NTST)
       CALL MPIBCAST1I(MPISTATE)
    ENDIF
    DO WHILE(MPISTATE/=CNRLBV_STOP)
...
! Compute the starting direction.
    ...
    IF(IAM==0.AND.NTST>NA)THEN
       UDOTPS(:,0:NCOL*NTST)=DUPS(:,0:NCOL*NTST)
    ENDIF
    DEALLOCATE(DUPS,DRL)
...
! Local
    ...
    INTEGER LABW,N2,NINS,NTOT,NTOTS,MPISTATE
