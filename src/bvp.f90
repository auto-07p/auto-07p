...
! THIS IS THE ENTRY ROUTINE FOR GENERAL BOUNDARY VALUE PROBLEMS.
    ...
    ELSE
       CALL MPI_CNRLBV_WORKER(AP,ICP,ICU,FUNI,BCNI,ICNI,FNCI)
    ENDIF
...
! Local
    ...
    INTEGER MPISTATE,IAM,KWT,NA,IFST,NLLV,NITSP1
    DOUBLE PRECISION DSOLD,RDS,P0(1),P1(1),THL(1),RDSZ,DET
    LOGICAL STEPPED
...
! Adapt the mesh to the solution.
       ...
       IF(AP%IAD/=0)THEN
          IF(MOD(AP%NTOT,AP%IAD)==0)THEN
             CALL MPIADAPT(NTST,NCOL,NDIM,UPS,DTM)
          ENDIF
       ENDIF
...
! Compute rate of change (along branch) of PAR(ICP(1)) and U :
       ...
       UDOTPS(:,0:NCOL*NTSTNA)=(UPS(:,0:NCOL*NTST)-UOLDPS(:,0:NCOL*NTST))/DSOLD
...
! Rescale, to set the norm of (UDOTPS,RLDOT) equal to 1.
    ...
    CALL MPICBV(AP%NPAR,PAR,AP%NFPR,RLDOT,RDS,NTST,NCOL,NDIM,UDOTPS)
