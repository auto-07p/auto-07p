...
! Local
    ...
    DOUBLE PRECISION, ALLOCATABLE :: UDOTPS(:,:)
    INTEGER NDIM,NTST,NCOL,NFPR,IPERP,ISTOP,NITPS,NODIR,NPAR
    INTEGER MPISTATE,IAM,KWT,NA,ICU(1)
    DOUBLE PRECISION DSOLD,DSOLDS(1),RDS,P0(1),P1(1),THL(1),TM(1)
...
! The stepsize used in the preceding step has been stored in DSOLD.
    ...
    DOUBLE PRECISION, INTENT(INOUT) :: RLOLD(AP%NFPR), RDS
...
! Local
    ...
    INTEGER LABW,N2,NINS,NTOT,NTOTS,IAM
...
! Check whether limits of the bifurcation diagram have been reached :
    ...
    IF(IPLT.GT.0.AND.IAB.LE.NDM)AMP=RMXUPSP(NA,NCOL,NDIM,IAB,UPS)
    IF(IPLT.GT.NDM.AND.IAB.LE.2*NDM.AND.IAM==0) &
         AMP=RINTG(NTST,NCOL,NDIM,IAB-NDM,UPS,DTM)
    IF(IPLT.GT.2*NDM.AND.IAB.LE.3*NDM.AND.IAM==0) &
...
! Compute maxima of solution components.
    ...
    DO I=1,N2
       ITMP=I
       IF(IAM==0)UMX(I)=RMXUPS(NTST,NCOL,NDIM,ITMP,UPS)
    ENDDO
