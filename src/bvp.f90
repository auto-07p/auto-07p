...
! The stepsize used in the preceding step has been stored in DSOLD.
    ...
    DOUBLE PRECISION, INTENT(IN) :: RLCUR(*), DTM(*), THL(*), THU(*)
    DOUBLE PRECISION, INTENT(INOUT) :: RLOLD(*)
    DOUBLE PRECISION, INTENT(OUT) :: RLDOT(*)
...
! Local
    ...
    DO J=0,NTST*NCOL
       CALL FUNI(AP,NDIM,UPS(:,J),UPS(:,J),ICP,PAR,0,UPOLDP(:,J),&
            DFDU,DFDP)
    ENDDO
...
! for the next solution (PAR(ICP(*)) , U) on a branch of solutions.
    ...
    DOUBLE PRECISION, INTENT(OUT) :: UPS(NDIM,0:*), RLCUR(*)
    DOUBLE PRECISION, INTENT(IN) :: UOLDPS(NDIM,0:*), RLOLD(*)
    DOUBLE PRECISION, INTENT(IN) :: UDOTPS(NDIM,0:*), RLDOT(*), UPOLDP(NDIM,0:*)
