...
! THIS IS THE ENTRY ROUTINE FOR GENERAL BOUNDARY VALUE PROBLEMS.
    ...
    ELSE
       CALL CNRLBV_WORKER(AP,ICP,ICU,FUNI,BCNI,ICNI,FNCI)
    ENDIF
...
! ---------- -----------------
  SUBROUTINE MPI_CNRLBV_WORKER(AP,ICP,ICU,FUNI,BCNI,ICNI,FNCI)
...
! Local
    ...
    DOUBLE PRECISION, ALLOCATABLE :: UDOTPS(:,:),TM(:)
    DOUBLE PRECISION, ALLOCATABLE :: DUPS(:,:),DRL(:)
    INTEGER NDIM,IPS,IRS,NTST,NCOL,ISW,NFPR
    INTEGER ISTOP,I,NITPS,NODIR,NPAR
    INTEGER MPISTATE,IAM,KWT,NA,IUZ(1),IFST,NLLV
    DOUBLE PRECISION DS,DSOLD,DSTEST,RDS,P0(1),P1(1),VUZ(1),THL(1),TEST,RDSZ,DET
    LOGICAL STEPPED
    CHARACTER(4) ATYPE
    COMPLEX(KIND(1.0D0)) EV(1)
...
! INITIALIZE COMPUTATION OF BRANCH
    ...
    ALLOCATE(UPOLDP(NDIM,0:NA*NCOL),UDOTPS(NDIM,0:NA*NCOL))
    ALLOCATE(TM(0:NA),DTM(NA))
...
!      ** Restart from orbit.
       ...
       ENDIF
       IF((NODIR==1.AND.ISW>0).OR.(IRS/=0.AND.ISW<0).OR. &
            (AP%ISP/=0 .AND. (IPS==2.OR.IPS==7.OR.IPS==12))) THEN
          ...
          CALL SOLVBV(IFST,AP,DET,PAR,ICP,FUNI,BCNI,ICNI,RDSZ,NLLV, &
               ...
               P0,P1,THL,THU)
          DEALLOCATE(DUPS,DRL)
          IF((NODIR==1.AND.ISW>0).OR.(IRS/=0.AND.ISW<0))THEN
             CALL MPIBCAST(RLDOT,NFPR)
             CALL MPISCAT(UDOTPS,NDIM*NCOL,NTST,NDIM)
          ENDIF
       ENDIF
...
! Store plotting data for restart point :
    ...
    DO WHILE(ISTOP==0)
       ...
       DO
          ...
          CALL LCSPBV(AP,DSOLD,DSTEST,PAR,ICP,MPISTATE,FUNI,BCNI,ICNI,FNCI, &
               TEST,RLCUR,RLOLD,RLDOT,NDIM,UPS,UOLDPS,UDOTPS, &
               UPOLDP,TM,DTM,P0,P1,EV,THL,THU,IUZ,VUZ,NITPS,ATYPE,STEPPED,NA)
       ENDDO
...
! Update UOLDPS, UDOTPS, UPOLDP, RLOLD, and RLDOT.
    ...
    ENDDO
    DEALLOCATE(PAR,THU)
    DEALLOCATE(UPS,UOLDPS,UPOLDP,UDOTPS,TM,DTM)
...
! INITIALIZE COMPUTATION OF BRANCH
    ...
    NODIR=0
    CALL RSPTBV(AP,PAR,ICP,FUNI,STPNBVI,FNCI,RLCUR,RLOLD, &
         RLDOT,NDIM,UPS,UOLDPS,UDOTPS,UPOLDP,TM,DTM,NODIR,THU,NA)
