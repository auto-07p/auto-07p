...
! INITIALIZE COMPUTATION OF BRANCH
    ...
    NODIR=0
    CALL RSPTBV(AP,PAR,ICP,FUNI,BCNI,ICNI,STPNBVI,FNCI,RLCUR,RLOLD, &
...
!     don't set global rotations here for homoclinics, but in autlib5.f
    ...
    IF(IPERP/=2)THEN
       CALL STDRBV(AP,PAR,ICP,FUNI,BCNI,ICNI,RLCUR,RLOLD,RLDOT,NDIM, &
            UPS,UOLDPS,UDOTPS,UPOLDP,DTM,IPERP,P0,P1,THL,THU,NTSTNA)
       IF(IPERP/=-1)THEN
...
! ---------- ------
  SUBROUTINE RSPTBV(AP,PAR,ICP,FUNI,BCNI,ICNI,STPNBVI,FNCI,RLCUR,RLOLD, &
       RLDOT,NDIM,UPS,UOLDPS,UDOTPS,UPOLDP,TM,DTM,NODIR,THU,NA)
...
! Get restart data :
    ...
    IF(IAM==0.AND.NCOLRS*NTSRS==0)THEN
...
! Set UOLDPS, RLOLD.
    ...
    NLLV=0
    CALL MPISBV(AP,PAR,NDIM,UPS,RLDOT,UDOTPS,DTM,THU,NLLV)
...
! the computation of a branch when no direction vector is given.
    ...
    TYPE(AUTOPARAMETERS), INTENT(IN) :: AP
    INTEGER ICP(*),NDIM,IPERP,NTSTNA
    DOUBLE PRECISION UDOTPS(NDIM,0:NTSTNA*AP%NCOL),DTM(*)
    DOUBLE PRECISION PAR(*),RLCUR(AP%NFPR),RLOLD(AP%NFPR),RLDOT(AP%NFPR)
    DOUBLE PRECISION THL(*),THU(*),UPS(NDIM,0:NTSTNA*AP%NCOL)
...
! in case the starting direction is to be determined.
    ...
    IF(IPERP==-1.OR.NTSTNA<NTST)THEN
       DEALLOCATE(DUPS,DRL)
       RETURN
    ENDIF
...
! This is used by BPCONT in periodic.f90 and toolboxbv.f90
    ...
    NLLV=1
    CALL MPISBV(AP,PAR,NDIM,UPS,RLDOT,UDOTPS,TM,THU,NLLV)
