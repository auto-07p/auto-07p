...
! Local
    INTEGER NCOL,NPAR,J
    DOUBLE PRECISION, ALLOCATABLE :: U(:),DFDU(:,:),DFDP(:,:)

    NCOL=AP%NCOL
    NPAR=AP%NPAR

    ALLOCATE(U(3*NDIM+AP%NFPR),DFDU(NDIM,NDIM),DFDP(NDIM,NPAR))
    DFDU(:,:)=0.d0
    DFDP(:,:)=0.d0
...
! ---------- ------
  SUBROUTINE LCSPBV(AP,DSOLD,DSTEST,PAR,ICP,ITEST,FUNI,BCNI,ICNI,FNCI,Q, &
       RLCUR,RLOLD,RLDOT,NDIM,UPS,UOLDPS,UDOTPS,UPOLDP, &
       TM,DTM,P0,P1,EV,THL,THU,IUZ,VUZ,NITPS,ATYPE,STEPPED,NTSTNA,MPISTATE)
...
! Return if tolerance has been met :
    ...
    ELSEIF(MPISTATE/=LCSPBV_CONT+ITEST)THEN
       RETURN
    ENDIF
...
! If requested write additional output on unit 9 :
       ...
       IF(IAM/=0)THEN
          CALL MPIBCAST1I(MPISTATE)
          IF(MPISTATE/=LCSPBV_CONT+ITEST)THEN
