...
! Local
    ...
    INTEGER NDIM,NTST,NCOL,NFPR,IPERP,ISTOP,I,NITPS,NODIR,NPAR
    INTEGER MPISTATE,IAM,KWT,NA,IFST,NLLV
    DOUBLE PRECISION DSOLD,RDS,P0(1),P1(1),THL(1),TM(1),RDSZ,DET
...
! Set UOLDPS, RLOLD.
    ...
    IF(IPERP/=2)THEN
       ...
       CALL MPICBV(NPAR,PAR,NFPR,RLDOT,RDS,NTST,NCOL,NDIM,UDOTPS) 
       IF(IPERP==0)THEN
          UDOTPS(:,:)=0.d0
          RLDOT(:)=0.d0
       ENDIF
...
! Local
    ...
    DOUBLE PRECISION PERIOD,TPI,RIMHB,T,C,S,THL(1),RLDOT(1)
...
! Note that the period-scaling in FUNC is taken into account:
    ...
    IF(IAM==0)THEN
       ALLOCATE(DTM(NTST))
       DTM(:)=1.d0/NTST
       CALL SCALEB(NTST,NCOL,NDIM,0,UDOTPS,RLDOT,DTM,THL,THU)
       DEALLOCATE(DTM)
    ENDIF
