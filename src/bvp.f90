...
! Local
    INTEGER NDIM,NARS,NSKIP,NSKIP1,NSKIP2,I,J
    ...
    NDIMRD=MIN(NDIM,NARS-1)

    NSKIP1=(NARS-1)/7 - NDIM/7
    DO J=1,NTSRS
       READ(3,*)TM(J),UPS(1:NDIMRD,J)
       IF(NSKIP1>0)CALL SKIP3(NSKIP1,EOF3)
       DO I=1,NCOLRS-1
          READ(3,*)UPS(I*NDIM+1,J),UPS(I*NDIM+1:I*NDIM+NDIMRD,J)
          IF(NSKIP1>0)CALL SKIP3(NSKIP1,EOF3)
       ENDDO
    ENDDO
    ...
    NSKIP=MAX(NFPRS-NFPR,0)
    READ(3,*)ICPRS(1:MIN(NFPR,NFPRS)),(J,I=1,NSKIP)
    READ(3,*)RLDOTRS(1:MIN(NFPR,NFPRS)),(PAR(1),I=1,NSKIP)

! Read U-dot (derivative with respect to arclength).

    NSKIP2=(NARS-2)/7 - (NDIM-1)/7
    DO J=1,NTSRS
       DO I=0,NCOLRS-1
          READ(3,*)UDOTPS(I*NDIM+1:I*NDIM+NDIMRD,J)
          IF(NSKIP2>0)CALL SKIP3(NSKIP2,EOF3)
       ENDDO
    ENDDO
    READ(3,*)UDOTPS(1:NDIMRD,NTSRS+1)
    IF(NSKIP2>0)CALL SKIP3(NSKIP2,EOF3)
