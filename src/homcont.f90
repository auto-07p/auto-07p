...
! Copy of common block for use by demos written in C
  TYPE,BIND(C) :: BLHOM_TYPE
     INTEGER(C_INT) :: ITWIST,ISTART,IEQUIB,NFIXED,NPSI,NUNSTAB,NSTAB,NREV
  END TYPE BLHOM_TYPE
  TYPE(BLHOM_TYPE), BIND(C) :: BLHOM_1
...
! ---------- ------
  SUBROUTINE AUTOHO(AP,ICP,ICU)
    ...
    USE AUTOMPI, ONLY : MPIIAM,MPIBCASTI,MPIBCASTAP
...
! ---------- ----
  SUBROUTINE ICHO(AP,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
    ...
    IF (JB<NINT) THEN
       IF(IJAC/=0)THEN
          ALLOCATE(DNT(NINT-JB,NDM+NPAR))
          CALL ICNI(AP,NDM,PAR,ICP,NINT-JB,U,UOLD,UDOT,UPOLD,F(JB+1),IJAC,DNT)
          DINT(JB+1:NINT,1:NDM)=DNT(1:NINT-JB,1:NDM)
          DINT(JB+1:NINT,NDM+1:NDIM)=0d0
          DINT(JB+1:NINT,NDIM+1:NDIM+NPAR)=DNT(1:NINT-JB,NDM+1:NDM+NPAR)
          DEALLOCATE(DNT)
       ELSE
          CALL ICNI(AP,NDM,PAR,ICP,NINT-JB,U,UOLD,UDOT,UPOLD,F(JB+1),0,DUM1)
       ENDIF
    END IF
...
! ---------- ----
  SUBROUTINE INHO(AP,ICP)
    ...
    BLHOM_1%NUNSTAB=NUNSTAB
    BLHOM_1%NSTAB=NSTAB
    BLHOM_1%IEQUIB=IEQUIB
    BLHOM_1%ITWIST=ITWIST
    BLHOM_1%ISTART=ISTART
    BLHOM_1%NREV=NREV
    BLHOM_1%NFIXED=NFIXED
    BLHOM_1%NPSI=NPSI
