...
! For periodic boundary conditions extrapolate by periodicity.
    ...
    IPER = (IPS.EQ.2 .AND. ABS(ISW).LE.1)
...
! intervals. The functions UPS and VPS are interpolated on new mesh.
    ...
    DOUBLE PRECISION, INTENT(IN) :: UPSR(NDOLD,0:*),VPSR(NDOLD,0:*),TMR(0:*)
...
! Generate the new mesh :
    ...
    DO J=1,NOLD
       DTM(J)=TMR(J)-TMR(J-1)
    ENDDO
    CALL NEWMSH(NDIM,NDX,UPSR,NOLD,NCOLD,TMR,DTM,NNEW,TM,IPER)
    DEALLOCATE(DTM)

! Replace UPS by its interpolant on the new mesh :

    CALL INTERP(NDIM,NOLD,NCOLD,TMR,UPSR,NDX,NNEW,NCNEW,TM,UPS)

! Replace VPS by its interpolant on the new mesh :

    CALL INTERP(NDIM,NOLD,NCOLD,TMR,VPSR,NDX,NNEW,NCNEW,TM,VPS)
...
! Redistributes the mesh according to the function EQDF.
    ...
    INTEGER, INTENT(IN) :: NDIM,NDX,NOLD,NCOLD,NNEW
    LOGICAL, INTENT(IN) :: IPER
...
! ---------- ----
  SUBROUTINE EQDF(NTST,NDIM,NCOL,DTM,UPS,NDX,EQF,IPER)
    ...
    INTEGER, INTENT(IN) :: NTST,NDIM,NCOL,NDX
    LOGICAL, INTENT(IN) :: IPER
