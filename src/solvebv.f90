...
!        CCLO avoids write overlap between threads
         ALLOCATE(DD(NFPR,NRC,MNT-1),FCFC(NRC,MNT-1),CCLO(NDIM,NRC,MNT-1))
      ENDIF
...
!     ---------- ------
      SUBROUTINE COPYCP(NA,NOV,NRA,NCA,A,NCB,B,NRC,C,A1,A2,BB,CC,CCLO,IT)
...
! Arguments
      ...
      DOUBLE PRECISION, INTENT(INOUT) :: CC(NOV,NRC,*),CCLO(NOV,NRC,*)
...
!     Reduce non-overlapping pieces
         CALL REDUCER(1,NTST,PLO,PHI,CCLO(1,1,IT),DDD(1,1,IT),FCFC(1,IT))
      ENDIF
...
! Arguments
       ...
       DOUBLE PRECISION, INTENT(INOUT) :: CCLO(NOV,NRC),DD(NCB,*),FC(*)
...
! Arguments
      ...
      DOUBLE PRECISION D(NCB,*),DDBC(NCB,*)
