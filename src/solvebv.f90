...
!     The matrices D and FC are unused in all nodes except the first.
      ...
      ! The REDUCED array monitors which threads have finished their
      ! part of REDUCE() so other threads know what to wait for.
      ! This array works in reverse for BCKSUB()
      ALLOCATE(REDUCED(NTST))
      REDUCED(:)=.FALSE.
...
!$OMP END PARALLEL
      ...
      DEALLOCATE(REDUCED,FCFC,FAA,SOL)
...
!$OMP BARRIER
      ...
      CALL REDUCE(A1,A2,BB,CC,C2,DD,FAA,FCFC,                           &
           NTST,NOV,NCB,NRC,S1,S2,IPC,IPR,IFST,NLLV,IT,NT,IAM,KWT,REDUCED)
...
! Backsubstitution in the reduction process.
      ...
      CALL BCKSUB(S1,A2,S2,BB,FAA,SOL,FC,NTST,NOV,NCB,IPC,IT,NT,IAM,KWT,REDUCED)
...
! Arguments
       ...
       DOUBLE PRECISION, INTENT(OUT)   :: S21(NOV,NOV),S12(NOV,NOV)
...
!      ---------- ------
       SUBROUTINE REDSWP(IC,NOV,NCB,S11,S12,A12,A21,S21,A22,BB1,BB2)

       INTEGER, INTENT(IN) :: IC,NOV,NCB
       DOUBLE PRECISION, INTENT(INOUT) :: S11(NOV),S12(NOV),A12(NOV),A21(NOV)
...
!      ---------- -------
       SUBROUTINE REDELIM(IC,NOV,NCB,IAMAX,JPIV,A12,A21,S12,S11,A22,S21,BB2,BB1)

       INTEGER, INTENT(IN) :: IC,NOV,NCB,JPIV
       INTEGER, INTENT(INOUT) :: IAMAX
       DOUBLE PRECISION, INTENT(INOUT) :: A12(NOV),S12(NOV),A22(NOV),BB2(NCB)
