...
! ---------- ------
  SUBROUTINE PVLSDS(AP,PAR,AA,RIMHB)
    ...
    DOUBLE PRECISION, INTENT(OUT) :: RIMHB
! Local
...
101 FORMAT(I4,I6,9X,'Hopf Function:',ES14.5)
  ...
  END SUBROUTINE PVLSDS

! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNHBDS(AP,ATYPE,RIMHB)

    USE SUPPORT, ONLY: CHECKSP

    TYPE(AUTOPARAMETERS), INTENT(INOUT) :: AP
    DOUBLE PRECISION, INTENT(IN) :: RIMHB
    CHARACTER(LEN=*), INTENT(OUT) :: ATYPE
! Local
    INTEGER, SAVE :: NINS
    INTEGER ISP,ITPST
    ...
    IF(LEN_TRIM(ATYPE)>0)THEN
       IF(NINS==AP%NINS)ATYPE=TRIM(ATYPE)//'0'
    ENDIF
100 NINS=AP%NINS
