...
! ---------- ----
  SUBROUTINE FNDS(AP,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    IF(AP%ITPST==8)THEN
       ...
       ! bottom of DFDU
       CALL FFNSX(AP,U,PAR,DFDU,DFDU(1,NDIM+1))
    ENDIF
...
! ---------- ----
  SUBROUTINE FNNS(AP,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FNHBF(AP,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP,FNDS)
...
! ---------- ------
  SUBROUTINE STPNNS(AP,PAR,ICP,U,UDOT,NODIR)
    ...
    CALL STPNHBF(AP,PAR,ICP,U,UDOT,NODIR,FNDS)
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNCSDS(AP,ICP,U,NDIM,PAR,ITEST,ITP) RESULT(Q)
    ...
    SELECT CASE(ITEST)
    CASE(4)
       ! Bogdanov-Takens for equilibria corresponds to a resonance
       ! (1:1 resonance on LP, 1:2 resonance on PD)
...
! ------ --------- -------- ------
  DOUBLE PRECISION FUNCTION FNHBDS(AP,U,PAR,ITP,AA)
...
! Compute the eigenvalues of the Jacobian
    ...
    IF(AP%ITPST==8)THEN
       ...
       THETA=U(AP%NDIM-1)
       REV=THETA*(THETA-PI(.5d0))*(THETA-PI(2d0/3))*(THETA-PI(1d0))
       ITP=TPSPAE(AP%EPSS,ITPST,PAR(11),THETA)
    ELSE
       ITP=TPSPAE(AP%EPSS,ITPST,PAR(11),0d0)
    ENDIF
    IF((ITPST/=0.AND.ITPST<8).OR..NOT.CHECKSP(LBTYPE(ITP),AP%IPS,AP%ILP,ISP))THEN
       FNHBDS=0d0
       ITP=0
    ELSE
       FNHBDS=REV
...
! ------- -------- ------
  INTEGER FUNCTION TPSPAE(EPSS,ITPST,PERIOD,THETA)
...
! Determines type of secondary bifurcation of maps.
    ...
    DOUBLE PRECISION, INTENT(IN) :: EPSS, PERIOD, THETA
