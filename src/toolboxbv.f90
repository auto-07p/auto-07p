...
! ---------- ----
  SUBROUTINE FNBL(AP,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FFBL(AP,U,UOLD,ICP,PAR,F,NDM,DFDU,DFDP)
...
! ---------- ----
  SUBROUTINE BCBL(AP,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
    ...
    DO I=1,NDIM
       UU=U0(I)
       U0(I)=UU-EP
       CALL FBBL(AP,NDIM,PAR,ICP,NBC0,U0,U1,FF1,DFU)
       U0(I)=UU+EP
...
! ---------- -----
  SUBROUTINE FNBBP(AP,NDIM,U,UOLD,ICP,PAR,IJAC,F,DFDU,DFDP)
    ...
    CALL FFBBP(AP,NDIM,U,UOLD,UOLD(NDIM+1),ICP,PAR,F,NDM,DFDU,DFDP)
    ...
    DO I=1,NDIM
       UU=U(I)
       U(I)=UU-EP
       CALL FFBBP(AP,NDIM,U,UOLD,UOLD(NDIM+1),ICP,PAR,FF1,NDM,DFU,DFP)
       U(I)=UU+EP
       CALL FFBBP(AP,NDIM,U,UOLD,UOLD(NDIM+1),ICP,PAR,FF2,NDM,DFU,DFP)
       U(I)=UU
       DO J=1,NDIM
          DFDU(J,I)=(FF2(J)-FF1(J))/(2*EP)
       ENDDO
...
! ---------- -----
  ...
  SUBROUTINE FIBBP(AP,NDIM,PAR,ICP,NINT,NNT0,U,UOLD,UDOT,UPOLD,FI,DINT)
    ...
    IF(ISW<0) THEN
       ...
       ENDDO
       DO I=1,NFPX
          FI(NFPX+4*NNT0+1)=FI(NFPX+4*NNT0+1)+PAR(NPARU+2*NFPX+NDM+1+I)**2
          FI(NFPX+4*NNT0+2)=FI(NFPX+4*NNT0+2)+PAR(NPARU+3*NFPX+NDM+1+I)**2
          FI(NFPX+4*NNT0+3)=FI(NFPX+4*NNT0+3)+PAR(NPARU+2*NFPX+NDM+1+I)* &
               PAR(NPARU+3*NFPX+NDM+1+I)
          FI(NFPX+4*NNT0+4)=FI(NFPX+4*NNT0+4)+PAR(NPARU+2*NFPX+NDM+1+I)* &
               PAR(NPARU+3*NFPX+NDM+1+I)
       ENDDO
