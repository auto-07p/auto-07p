...
! ---------- -----
  SUBROUTINE FFBBP(AP,NDIM,U,UOLD,UPOLD,ICP,PAR,F,NDM,DFDU,DFDP)
    ...
    IF(NNT0>0) THEN
       ALLOCATE(FI(NNT0),DINT(NNT0,NDM+AP%NPAR))
...
! ---------- -----
  SUBROUTINE BCBBP(AP,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
    ...
    IF(NDIM==NDM)THEN
       CALL BCNI(AP,NDIM,PAR,ICP,NBC,U0,U1,F,IJAC,DBC)
       RETURN
    ENDIF
...
! ---------- -----
  SUBROUTINE ICBBP(AP,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
    ...
    IF(NDIM==NDM)THEN
       CALL ICNI(AP,NDIM,PAR,ICP,NINT,U,UOLD,UDOT,UPOLD,F,IJAC,DINT)
       RETURN
    ENDIF
...
! ---------- -------
  SUBROUTINE STPNBBP(AP,PAR,ICP,NTSR,NCOLRS,RLDOT,UPS,UDOTPS,TM,NODIR)
    ...
    DOUBLE PRECISION, ALLOCATABLE :: UPSR(:,:),UDOTPSR(:,:),TMR(:)
    INTEGER NDIM,ISW,NBC,NINT,NFPR
    ...
    CALL STSDRBV(AP2,PAR,ICPRS,FNBBP,BCBBP,ICBBP, &
    ...
    DEALLOCATE(UPST,UDOTPST,VDOTPST,RVDOT)
