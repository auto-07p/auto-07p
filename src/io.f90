...
MODULE IO
  ...
  TYPE SOLUTION
     ...
     DOUBLE PRECISION, DIMENSION(:), POINTER :: TM, RLDOT, PAR
...
! If the label can not be located on unit 3 then FOUND will be .FALSE.
    ...
    DO
       ...
       IF(LAB.EQ.IRS.AND..NOT.FOUND)THEN
          FOUND=.TRUE.
          IAP(27)=ITP
          IAP(30)=IBR
...
! Local
    INTEGER NDIM, J
    DOUBLE PRECISION, ALLOCATABLE, TARGET, SAVE :: UPS(:,:), UDOTPS(:,:), &
         TM(:), RLDOT(:), PAR(:)
    INTEGER, ALLOCATABLE, TARGET, SAVE :: ICP(:)

    NULLIFY(ROOTSOL)
    NDIM=NAR-1

    ALLOCATE(UPS(NDIM,NTPL),TM(NTPL))
    ALLOCATE(UDOTPS(NDIM,NTPL))
    DO J=1,NTPL
       READ(3,*)TM(J),UPS(:,J)
    ENDDO

    IF(NTST>0)THEN
       ALLOCATE(ICP(NFPR),RLDOT(NFPR))
       READ(3,*)ICP(:)
       READ(3,*)RLDOT(:)
...
! Read the parameter values.
    ...
    CALL NEWSOL(IBR,NTOT,ITP,LAB,NFPR,ISW,NTPL,NAR,NROWPR,NTST,NCOL,NPAR, &
         TM,UPS,ICP,RLDOT,UDOTPS,PAR)

  END SUBROUTINE READSOL

! ---------- ------
  SUBROUTINE NEWSOL(IBR,NTOT,ITP,LAB,NFPR,ISW,NTPL,NAR,NROWPR,NTST, &
       NCOL,NPAR,TM,UPS,ICP,RLDOT,UDOTPS,PAR)

    INTEGER, INTENT(IN) :: IBR,NTOT,ITP,LAB,NFPR,ISW,NTPL,NAR,NROWPR,&
         NTST,NCOL,NPAR
    DOUBLE PRECISION, TARGET, INTENT(IN), &
         DIMENSION(:,:) :: UPS, UDOTPS
    DOUBLE PRECISION, TARGET, INTENT(IN) :: TM(:), PAR(:), RLDOT(:)
    INTEGER, TARGET, INTENT(IN) :: ICP(:)
...
! Local
    INTEGER NDIM,I,J,NFPR,NFPRS,NPARR
    ...
    DO J=1,NTSRS
       TM(J)=CURSOL%TM((J-1)*NCOLRS+1)
       UPS(1:NDIMRD,J)=CURSOL%UPS(1:NDIMRD,(J-1)*NCOLRS+1)
       DO I=1,NCOLRS-1
          UPS(I*NDIM+1:I*NDIM+NDIMRD,J)= &
               CURSOL%UPS(1:NDIMRD,(J-1)*NCOLRS+I+1)
       ENDDO
    ENDDO
    TM(NTSRS+1)=CURSOL%TM(NTSRS*NCOLRS+1)
    UPS(1:NDIMRD,NTSRS+1)=CURSOL%UPS(1:NDIMRD,NTSRS*NCOLRS+1)
...
! Read U-dot (derivative with respect to arclength).
    ...
    DO J=1,NTSRS
       DO I=0,NCOLRS-1
          UDOTPS(I*NDIM+1:I*NDIM+NDIMRD,J)= &
               CURSOL%UDOTPS(1:NDIMRD,(J-1)*NCOLRS+I+1)
       ENDDO
    ENDDO
    UDOTPS(1:NDIMRD,NTSRS+1)=CURSOL%UDOTPS(1:NDIMRD,NTSRS*NCOLRS+1)
