...
! Local
    ...
    ALLOCATE(UPS(NDIM,0:NTNC),TM(0:NTNC),UDOTPS(NDIM,0:NTNC))
    DO J=0,NTNC
       READ(3,*)TM(J),UPS(:,J)
    ENDDO
...
! ---------- ------
  SUBROUTINE NEWSOL(IBR,NTOT,ITP,LAB,NFPR,ISW,NTPL,NAR,NROWPR,NTST, &
    ...
    DOUBLE PRECISION, TARGET, INTENT(IN), &
         DIMENSION(:,0:) :: UPS, UDOTPS
    DOUBLE PRECISION, TARGET, INTENT(IN) :: TM(0:), PAR(1:), RLDOT(1:)
...
! Local
    INTEGER NDIM,J,NFPR,NFPRS,NPARR
