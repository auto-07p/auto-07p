...
The {\cal AUTO} file {\tt auto07p-0.1.tar.gz} is
available via HTTP from {\it http://sourceforge.net/projects/auto2000}.
The {\tt README} file contains the instructions for printing this manual.
Below it is assumed that you are using the Unix (e.g. {\it bash}) shell 
and that the file {\tt auto07p-0.1.tar.gz} is in your main directory.

While in your main directory, enter the commands
\commandf{gunzip auto07p-0.1.tar.gz},
followed by\\
\commandf{tar xvfo auto07p-0.1.tar}.
...
and {\cal Motif}.
Note that the GUI is not strictly necessary, since {\cal AUTO} can be run very effectively using the Unix Command Language User Interface (CLUI).
...
and then \filef{ make} in the directory \filef{ auto/07p}.
...
Inventor 2.5.10or Coin3D 2.2 with SoXt 1.1.0. It also needs the
...
For timing purposes, the file {\tt auto/07p/src/autlib1.f}
contains references to the function {\it etime}.
If this function is not automatically supplied by your Fortran compiler then it can be 
...
To recompile {\tt autlib1.f}, type {\it @C 1} in directory {\tt
  auto/07p/src}. Similarly, this file contains references to the
subroutine {\it FLUSH} to avoid buffering of screen output when
running \AUTO from Python. These calls can be commented out if your
Fortran compiler does not support them.

The PostScript conversion command {\it @ps} is compiled by
default. Alternatively you can type {\it make} in directory.  {\tt auto/07p/tek2ps}.
To generate the on-line manual, type {\it make} in {\tt auto/07p/doc}.

To prepare {\cal AUTO} for transfer to another machine, type {\it make superclean}
in directory {\tt auto/07p} before creating the {\it tar}-file. 
...
itself.
\item
Install Gfortran from \webf{http://wiki.gnu.org/Gfortran}.
\item
...
\webf{http://wiki.gnu.org/Gfortran}, which contains binaries for
...
it can use OpenMP to run most of its code in parallel
...
in serial mode you just type \commandf{ export OMP\_NUM\_THREADS=1}.  To run the
same command in parallel on 4 processors you type 
\commandf{export OMP\_NUM\_THREADS=4}. Without any {\tt OMP\_NUM\_THREADS}
set its number is equal to the actual number of processors
on the system.
...
and {\tt exp/r.exp} in {\tt auto/07p/demos}.
See Chapter~\ref{ch:AUTO_constants}
...
${\tt NTST}\times{\tt NDIM}\times({\tt NCOL}\times{\tt NDIM}+{\tt NBC}+{\tt NINT})$. 
